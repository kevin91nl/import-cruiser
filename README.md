...
Run `make self-graph` to refresh these artifacts—the script passes `--show-loc` so every cluster (including `src` and `import_cruiser`) includes its LOC badge in the DOT/HTML/SVG exports.
The target also passes `--include-external-deps`, so non-dev third-party packages render as a grouped green block labeled “External dependencies” (without LOC badges).
